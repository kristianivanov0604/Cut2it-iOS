...
- (void)authorize:(NSArray *)permissions
	...
	if ([self isSessionValid] && [delegate respondsToSelector:@selector(fbDidLogin:)]) {
		[delegate fbDidLogin:self.accessToken];
		return;
	}
...
- (void)fbDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate {
  ...
  self.expirationDate = expirationDate;
    if ([self.sessionDelegate respondsToSelector:@selector(fbDidLogin:)]) {
        [_sessionDelegate fbDidLogin:self.accessToken];
  }
...
- (void) onCloseDialog{
 	if ([_sessionDelegate respondsToSelector:@selector(onCloseDialog)]) {
		[_sessionDelegate onCloseDialog];
    }	
}
